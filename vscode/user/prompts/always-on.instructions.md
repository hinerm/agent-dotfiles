...
- For external Java API or resource lookups, first check whether Serena can query the API from the active project or a registered local source checkout. Use Serena's symbol tools when the source is available there.
- If Serena cannot provide the source, invoke the `dependency-source-research` agent when it is available. Provide the workspace path and dependency, symbol, or resource to investigate, then use its report as evidence before exploring broadly. If it finds a useful local checkout, suggest registering that checkout with Serena for future lookups. This delegation rule does not apply inside that research agent.
- Serena uses the active project for the current workspace and additional registered projects for local dependency source checkouts. With `query-projects` enabled and Serena's ProjectServer running, use read-only project queries to inspect those external sources without changing the active project.
